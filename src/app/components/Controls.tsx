...
const DiffWithPreviousButton = ({ gameVer }: { gameVer: GameVersion }) => {
  ...
  const prevVersion = Version.apply(
    gameVer.version.major,
    Math.max(gameVer.version.minor - 1, 0),
  );
  ...
  return (
    <Link
      ...
      href={`/${gameVer.gameName}/cards/${Version.toString(gameVer.version)}/diff/${prevVersionString}`}
    >
...
const FilterBox = ({ setInput, ...props }: FilterBoxProps) => {
  const ClearInput = () => (
    <a
      className="absolute pt-1 right-1 z-0 cursor-pointer"
      onClick={() => setInput("")}
    >
      x
    </a>
    ...
    <div>
      ...
      {!hidden && (
        <code className="text-xs">
          (key:value AND key2!value|value2 OR key3:&quot;value with
          spaces&quot;)
        </code>
      )}
    </div>
...
export default function Controls({ gameVer, filterQuery }: ControlsProps) {
  ...
  const filterLink = () => {
    ...
    const filterParam =
      filterBuilder.query.length > 0 ? [`q=${filterBuilder.query}`] : [];
    return [
      hrefWithoutParams,
      filterParam.concat(withoutQueryParam).join("&"),
    ].join("?");
  };
