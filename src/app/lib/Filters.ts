...
const getKeyInsensitive = (a: Filterable, key: string): string | undefined =>
  Object.keys(a).find((k) => eqInsensitive(k, key));
const accessWithKeyInsensitive = (
  a: Filterable,
  key: string,
): any | undefined => {
    ...
    .map(
      (or) =>
        new Condition<Filterable>((a) =>
          cmp(accessWithKeyInsensitive(a, k) || "", or),
        ),
    )
    .reduce(combine);
}
